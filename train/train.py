...
hidden_size = 16  # LSTM和GCN模型的隐藏层维度
num_layers = 1
output_size = 2
...
for i in range(0, timestep, slide_step):  # 对于从0到timestep，每一次滑动slide_step来说，i为；
    if i + history_window + pred_window >= timestep or i + history_window >= timestep:  # 如果timestep给的太小了或者结束预测
        break  # 结束
    # 如果没有结束预测则有如下：
    x_data = data[:, i:i + history_window].reshape((num_nodes, history_window))
    macro_x_data = macro_dynamic_feat[:, i:i + history_window, :].reshape((num_communities, history_window, 2))
    x.append(x_data)
    macro_x.append(macro_x_data)
    y.append(data[:, i + history_window + pred_window].reshape(num_nodes, 1))
    macro_y.append(macro_dynamic_feat[:, i + history_window + pred_window, :].reshape(num_communities, 2))

x = np.array(x, dtype=np.float32).transpose((1, 0, 2))  # 转置
y = np.array(y, dtype=np.float32).transpose((1, 0, 2))  # 将其都进行转置，两个元素的是进行行列转置，三个元素的就是将loc和time进行转置
...
weight = class_weight.compute_class_weight(class_weight='balanced', classes=np.unique(all_m_y), y=all_m_y)
print(weight)
...
class CustomLoss(nn.Module):
    ...
    def forward(self, micro_inputs, micro_outputs, micro_targets, macro_targets, macro_outputs):
        # 计算交叉熵损失
...
model = Net(input_size, hidden_size, num_layers, output_size, history_window, device).to(device)
...
all_loss = []
all_test_loss = []
all_train_Acc_0, all_train_Acc_1, all_train_Precision, all_train_Recall, all_train_F1 = [], [], [], [], []
...
def cal_0_to_1(input, result, label):
    ...
    F1 = np.mean(F1)
    return label_0, label_1, pred_00, pred_01, pred_10, pred_11, Acc_0, Acc_1, Precison, Recall, F1


F1_best = 0
for epoch in range(epoch):  # 对于每一次的epoch
    print('epoch:', epoch)
    model.train()  # 标志下面为训练阶段

    optimizer.zero_grad()  # 梯度清零

    micro_output, macro_output, varibales = model(x, edges, node_degrees, com, com_edges, com_weights, macro_x,
                                                  macro_static_feat,
                                                  k_in)
    micro_output = micro_output.permute(1, 2, 0)
    macro_output = macro_output.permute(1, 0, 2)
    micro_y = torch.argmax(y, dim=1)
    train_loss = criterion(x[train_mask], micro_output[train_mask], micro_y[train_mask], macro_y, macro_output)
    print('train loss', train_loss)

    label_0, label_1, \
    pred_00, pred_01, pred_10, pred_11, \
    train_Acc_0, train_Acc_1, \
    train_Precision, train_Recall, train_F1 = cal_0_to_1(x[train_mask], micro_output[train_mask], y[train_mask])

    model.eval()

    test_loss = criterion(x[test_mask], micro_output[test_mask], micro_y[test_mask], macro_y, macro_output)
    all_test_loss.append(test_loss.item())
    print('test loss', test_loss)
    label_0, label_1, \
    pred_00, pred_01, pred_10, pred_11, \
    test_Acc_0, test_Acc_1, \
    test_Precision, test_Recall, test_F1 = cal_0_to_1(x[test_mask], micro_output[test_mask], y[test_mask])

    writer.add_scalars('Loss', {'train': train_loss.item(), 'test': test_loss.item()}, epoch)
    ...
    writer.add_scalars('Precision', {'train': train_Precision, 'test': test_Precision}, epoch)
    writer.add_scalars('Recall', {'train': train_Recall, 'test': test_Recall}, epoch)
    writer.add_scalars('F1', {'train': np.mean(train_F1), 'test': np.mean(test_F1)}, epoch)
